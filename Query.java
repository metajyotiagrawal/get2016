...
public class Query {
	...
	/*
	 * Problem1 to fetch all the books published by author, given the name of
	 * the author. Returns the list of books data
	 */
	public ArrayList<Titles> getBookTitle() throws SQLException {
		...
		String query1 = "SELECT t.title_id,t.title_name,subject_id,publisher_id FROM titles t JOIN title_author ta ON ta.title_id=t.title_id JOIN authors a ON a.author_id = ta.author_id WHERE a.author_nm ='"
				+ authorName + "'";
		ResultSet rs1 = connect.getconnection().executeQuery(query1);
	...
	/*
	 * Problem2 Given the name of the book, to be issued by an existing member
	...
	public boolean getMemberNameByBookTitle() {
		...
		try {
			...
			ResultSet rs2 = connect.getconnection().executeQuery(query2);
			// to check if result set is empty
			if (rs2.next() == false) {
				System.out
	...
	/*
	 * Problem3 to delete all those books which were not issued in last 1 year
	...
	public int DeleteBooks() {
		...
		try {
			...
			int rs3 = connect.getconnection().executeUpdate(query3);
			String query4 = "SELECT * FROM Book";

			ResultSet rs4 = connect.getconnection().executeQuery(query4);
